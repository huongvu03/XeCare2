...
public interface GarageServiceRepository extends JpaRepository<GarageService, Long> {
    ...
    Optional<GarageService> findByGarageAndService(Garage garage, Service service);
    
    // Tìm tất cả garage cung cấp một dịch vụ cụ thể
    List<GarageService> findByServiceAndIsActiveTrue(Service service);
    
    // Kiểm tra garage đã có dịch vụ này chưa
    boolean existsByGarageAndService(Garage garage, Service service);
} 